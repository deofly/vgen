# LTX-2.5 distilled GGUF Q4 text-to-video 1.0.2
...
`vgen/comfyui-gguf@1.0.0` Node Pack automatically through signed Worker
maintenance. Its source, Windows Python wheels, artifact digest, Git revision,
and complete loaded node-class set are immutable. The Worker pauses only
ComfyUI, validates `/object_info`, and restores the previous node directory if
activation fails.
