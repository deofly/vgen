...
# Public authentication endpoints have a proxy-side limit in addition to the
# bounded in-process limiter. These zones live in conf.d's http context.
...
server {
  ...
  # Control-plane requests are JSON metadata. Task media goes directly to OSS;
  # the local capability route below is the only large streaming exception.
  ...
  # Never append an untrusted client-supplied forwarding chain. Uvicorn only
  # accepts this header from the loopback Nginx peer.
...
EOF
  ...
  location = /releases/install-windows-worker.ps1 {
    alias /var/www/vgen-releases/install-windows-worker.ps1;
...
EOF
  ...
  location ^~ /api/v1/artifacts/transfer/ {
    # The signed ticket is verified before the app consumes the stream, and
    # its own max_bytes is authoritative. Never aggregate this body in memory
    # or Nginx temporary files.
  ...
  # Nginx-generated rejections use the same machine contract as application
  # errors. proxy_intercept_errors remains off, so typed upstream 413/429
  # responses (including their dynamic Retry-After) pass through unchanged.
...
render_legacy_nginx_config_profile() {
  # Exact pre-hardening profile accepted only as an upgrade source. Keeping a
  # deterministic renderer lets upgrade distinguish our old config from an
  # operator-edited virtual host before replacing it atomically.
  local destination="$1"
  local include_public_releases="$2"
  cat >"${destination}" <<EOF
# Generated by the VGen Gateway installer for ${DOMAIN}.
...
render_previous_gateway_nginx_config() {
  # The immediately previous release already used a separate release hostname.
  render_legacy_nginx_config_profile "$1" 0
}

render_split_domain_legacy_gateway_nginx_config() {
  ...
  render_legacy_nginx_config_profile "$1" 1
}
...
verify_upgrade_preconditions() {
  ...
  local expected_config previous_config split_domain_legacy_config version_order
  ...
  split_domain_legacy_config="$(mktemp \
    "${NGINX_CONFIG_PATH}.upgrade-split-domain-legacy.XXXXXX")"
  render_nginx_config "${expected_config}"
  render_previous_gateway_nginx_config "${previous_config}"
  render_split_domain_legacy_gateway_nginx_config "${split_domain_legacy_config}"
  if ! cmp --silent -- "${NGINX_CONFIG_PATH}" "${expected_config}" && \
     ! cmp --silent -- "${NGINX_CONFIG_PATH}" "${previous_config}" && \
     ! cmp --silent -- "${NGINX_CONFIG_PATH}" "${split_domain_legacy_config}"; then
    rm -f -- "${expected_config}" "${previous_config}" "${split_domain_legacy_config}"
