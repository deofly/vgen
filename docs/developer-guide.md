...
| Gateway | 元数据、调度、准入、租约、审计、用量的唯一控制面 | v1 为单进程 SQLite WAL |
...
- Workspace Owner/Admin 管理非加密成员状态和所有 Pool；v1 不另设 Pool Admin。只有
...
v1 接受单 Gateway/SQLite 单点：Gateway 不可用时不能创建准入、提交新任务或领取新 lease；
Worker 可以完成已开始的本地执行并 journal 结果等待重试。跨 Gateway 联邦、负载均衡写入和
active-active 不属于 v1。
...
Owner pin 和 admission 链验证。允许 v1 历史 envelope 只读解密，但不得用裸 Gateway 公钥创建
新的 v1 grant。
...
记录敏感请求体。Pydantic 请求模型拒绝未知字段；客户端可容忍 v1 内增加响应字段，但必须拒绝
...
一个 Worker ID 在 v1 只启用一个 Executor 实例；多 GPU 主机可以运行多个独立 Worker 进程。
...
## 10. v0 到 v1 的 breaking migration

v1 不保留旧 `/api/*`、`CLIENT_TOKEN`/`WORKER_TOKEN`、graph-in-lease 或旧 task history。旧部署
只能作为离线 custom workflow 来源：
...
5. Converter 只写 `custom/` package，不导入 DB/task/token/prompt/media/model/cloud credential，
   也不覆盖现有 package。
6. 建立全新 v1 DB，重新初始化身份、Workspace、Pool、Worker、allocation、rate 和工作流。
7. 完成 L4/L5 验收后切换公网 `/api/v1`，销毁旧 token 并移除旧自启服务。

回滚是服务级恢复，不是数据库降级：停止 v1、保留 v1 为只读 incident record；业务允许时从
隔离的 v0 备份恢复旧服务并签发全新旧凭据。v1 身份、Task、envelope 和 usage 不得写回 v0。
